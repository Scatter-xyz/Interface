...
const Create = () => {
    ...
    const mintNFT = async (ipfsLink) => {
        console.log("Walet Context: ", wallet);
        const mintContract = new ethers.Contract(wallet.chain.mintContract, mintABI, wallet.provider);
        const signedMintContract = mintContract.connect(wallet.signer);
    ...
    return (
        <div className="w-full min-h-content bg-gin-50">
            <Navbar pageLoad="Create" setWallet={setWallet}/>
